"""OpenTelemetry instrumentation wiring for dispatch-mcp (#743).

dispatch-mcp is instrumented via OpenTelemetry zero-code auto-instrumentation
(`opentelemetry-instrument dispatch-mcp`), kept vendor-neutral via OTEL_* env.
...
def test_readme_documents_opentelemetry_instrument_startup() -> None:
    assert "opentelemetry-instrument dispatch-mcp" in README
